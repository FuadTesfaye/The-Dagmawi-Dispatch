...
// Helper: get today's date string in EAT (UTC+3)
...
// Global error handler — ensures the webhook always returns 200 to Telegram
bot.catch((err) => {
...
bot.command("subscribe", async (ctx) => {
  try {
    ...
    await ctx.reply("🕊️ *It is done!* The royal pigeon will deliver the digest to you every morning. Selam (peace) be with you.", { parse_mode: "Markdown" });
  } catch (err) {
    console.error("subscribe error:", err);
    await ctx.reply("⚠️ The royal pigeon got lost. Please try /subscribe again.");
  }
});
...
bot.command("unsubscribe", async (ctx) => {
  try {
    ...
    await ctx.reply("❌ *You have been banished from the pigeon route.* No more digests for you. Ayzosh (take courage) on your own!", { parse_mode: "Markdown" });
  } catch (err) {
...
bot.command("yesterday", async (ctx) => {
  try {
    ...
    await ctx.reply("⚠️ The herald tripped over his scrolls. Please try /yesterday again in a moment.");
  }
});
...
bot.command("date", async (ctx) => {
  try {
    ...
    await ctx.reply("⚠️ The herald tripped over his scrolls. Please try again.");
  }
});
...
bot.command("babiometer", async (ctx) => {
  try {
    ...
    await ctx.reply(`*The Babi-o-meter for today:*\n\n${blasts}\n_${count} posts today._\n\n${message}`, { parse_mode: "Markdown" });
  } catch (err) {
    console.error("babiometer error:", err);
    await ctx.reply("⚠️ The royal measuring device malfunctioned. Try /babiometer again.");
  }
});
