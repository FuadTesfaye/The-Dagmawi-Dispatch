...
async function main() {
  ...
  ok(`roast generated for dagmawi_babi`, typeof roast === 'string' && roast.length > 0 && roast.length <= 320 && !roast.includes('\n'));
  console.log(`  Roast: "${roast}"`);

  const roastOther = generateStaticRoast('cooliocodes');
  ok(`roast works for other channels`, typeof roastOther === 'string' && roastOther.length > 0 && roastOther.length <= 320);
