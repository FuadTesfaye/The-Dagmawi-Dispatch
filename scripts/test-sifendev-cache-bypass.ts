...
async function run() {
  ...
  const commands = [
    ...
    // /babiometer checks the posts table directly, ignoring dailySummaries cache
    { cmd: "/babiometer", expected: 1, wait: 8000 },
    // A completely new date that was never queried before to avoid cache
    { cmd: "/date 2026-08-11", expected: 1, wait: 20000 },
  ];
