...
export function CommentDrawer({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast('Comment published to the realm!', 'success');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-zinc-950/80 backdrop-blur-sm animate-in fade-in">
      <div className="relative w-full max-w-md h-full glass-panel bg-zinc-950/98 border-l border-zinc-800 p-5 shadow-2xl flex flex-col justify-between">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto py-4 flex flex-col gap-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center text-zinc-500 gap-2">
              <span>📜</span>
              <p className="text-xs">No comments yet. Be the first to speak in court!</p>
            </div>
          ) : (
            comments.map((c) => (
              <div
                key={c.id}
                className="p-3.5 rounded-2xl bg-zinc-900/60 border border-zinc-800/80 flex flex-col gap-2 transition-colors hover:border-zinc-700/60"
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-[11px] text-zinc-500">{formatTimeAgo(c.createdAt)}</span>
                </div>
                ...
                <button
                  ...
                  className="text-[11px] text-zinc-500 hover:text-amber-400 self-start flex items-center gap-1 mt-1 transition-colors"
                >
        ...
        {/* Comment Composer */}
        <form onSubmit={handleSubmit} className="pt-3 border-t border-zinc-800 flex flex-col gap-2">
          {replyTo && (
            <div className="flex items-center justify-between px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-300">
          ...
          <div className="relative flex items-center">
            <input
              ...
              className="w-full py-3 pl-4 pr-12 rounded-2xl bg-zinc-900 border border-zinc-800 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-amber-500/50 disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={!user || !newContent.trim() || submitting}
              className="absolute right-2 p-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 disabled:opacity-30 transition-all"
            >
              {submitting ? (
